...
k_clusters = 20      # Количество кластеров K-Means
blur_radius = 2     # Радиус размытия (фильтр шума)
min_d2 = 850        # Базовый порог расстояния цвета
...
def open_file(karta_input):
    ...
    # Применяем размытие только если радиус > 0
    if blur_radius > 0:
...
def run_kmeans(data, k=20, max_iters=10):
    ...
    if not data:
        return []
    # Быстрая выборка для инициализации центроидов
    ...
    for iteration in range(max_iters):
        ...
        # Для ускорения на чистом Python берем подмножество пикселей при обучении центроидов
...
def filter_isolated_pixels():
    """Фильтрация одиночных пикселей-сирот для очистки карты от шума"""
    global sp_sloy, x, y
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    ...
                    if (px + dx, py + dy) in s.sp_pix:
                        neighbors += 1
            if neighbors >= 2:  # Оставляем пиксель, если у него есть хотя бы 2 соседа того же слоя
                filtered_pix.add(pt)
...
def load_project(project_name="project_save.json"):
    ...
    sp_sloy.clear()
    for layer_dict in project_data["layers"]: sp_sloy.append(clas.Sloy.from_dict(layer_dict))
...
def main_all(karta_input):
    ...
    for idx, color in enumerate(centroids):
        # Назначаем каждому слою его базовый порог max_d2 из настроек
...
# Добавьте эту функцию в конец файла main.py

def add_new_empty_layer(name, rgb_color):
    """Регистрирует новый слой в глобальном списке приложения"""
